(() => {
  ...
  $("#btnUpload").addEventListener("click", async () => {
    ...
    try {
      ...
      form.append("roster", state.files.roster);
  ...
  $("#btnMappingPreview").addEventListener("click", async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "欄位偵測失敗");
  ...
  $("#btnRun").addEventListener("click", async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "比對執行失敗");
  ...
  function renderResults(data) {
    ...
    $("#downloadPrimary").href = `/api/download/${state.runId}/${encodeURIComponent(data.downloads.primary)}`;
    $("#downloadUnmatched").href = `/api/download/${state.runId}/${encodeURIComponent(data.downloads.unmatched)}`;
  }
